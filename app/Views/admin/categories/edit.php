...
<div class="card shadow mb-4">
    <div class="card-body">
        <form method="POST" action="<?= base_url('admin/categories/' . $category->id . '/edit') ?>">
            <?= csrf_field() ?>
            <div class="form-group"><label><?= lang('Admin.categoryName') ?> *</label><input type="text" name="name" class="form-control" required value="<?= esc($category->name) ?>"></div>
            <div class="form-group"><label><?= lang('Admin.categoryDescription') ?></label><textarea name="description" class="form-control" rows="3"><?= esc($category->description) ?></textarea></div>
