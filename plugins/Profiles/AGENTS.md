...
5. **Admin other-user editing keys on `profile.edit.any`.** The owner can always edit themselves; editing someone else requires the permission (`Controllers/ProfilesAdminController.php:29-33, 57-61`). Admin routes are gated on `admin.access` automatically by the core; the in-controller `profile.edit.any` checks are the per-action enforcement layer.
6. **Every posted redirect goes through `UrlService::sameSite()`.** `update()` passes the posted `return_url` through `sameSite()` before redirecting (`Controllers/ProfilesAdminController.php:67, 72`), so a hostile value can never reach a `Location` header. Keep that choke point when adding new posts.
