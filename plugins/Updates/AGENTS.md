...
13. **An applied release drops the cached check.** `UpdateApplyService::apply()` calls `UpdateService::invalidateCheckCache()` once the copy phase has run, which clears `Updates.lastCheckAt` so the next read fetches the feed instead of reporting the pre-update version. The drop sits at the copy, not at the end, because a later phase can fail and leave the copied files in place. Reason: the cached state names the installed version, and an update makes it wrong.
